...
const crimeColorScale = d3.scaleOrdinal().domain([
    'HOMICIDE',
    'BATTERY',
    'ASSAULT',
    'ROBBERY',
...
// create svg
const svg = d3.select("#map").attr("width", config.width).attr("height", config.height);
...
const projection = d3.geoMercator().center([
    -87.65,
    41.85
]).scale(80000).translate([
    config.width / 2,
    config.height / 2
]);
...
async function loadChicagoMap() {
    try {
        ...
        // draw neighborhoods
        mapLayer.selectAll("path").data(geoData.features).join("path").attr("class", "neighborhood").attr("d", path).on("mouseover", function(event, d) {
            d3.select(this).attr("fill", "rgba(140, 160, 190, 0.6)");
            if (d.properties && d.properties.name) tooltip.style("display", "block").html(`<strong>${d.properties.name}</strong>`).style("left", event.pageX + 10 + "px").style("top", event.pageY - 10 + "px");
...
function drawSimplifiedChicago() {
    ...
    mapLayer.append("path").datum(chicagoBounds).attr("class", "neighborhood").attr("d", path).style("fill", "rgba(100, 120, 150, 0.2)").style("stroke", "rgba(255, 255, 255, 0.5)").style("stroke-width", "2px");
}
...
function updateVisualization(periodIndex) {
    ...
    // --- CRIME SCATTERS ---
    // Project coordinates
    const crimePointsData = crimePeriod.locations.map((d)=>({
            ...d,
            x: projection([
                d.lon,
                d.lat
    ...
    // Bind data
    const crimeCircles = crimeLayer.selectAll(".crime-dot").data(crimePointsData, (d, i)=>i); // Use index as key for simple transition
    // Exit
    crimeCircles.exit().transition().duration(200).attr("r", 0).attr("opacity", 0).remove();
    // Enter
    const crimeEnter = crimeCircles.enter().append("circle").attr("class", "crime-dot").attr("cx", (d)=>d.x).attr("cy", (d)=>d.y).attr("r", 0).attr("fill", (d)=>crimeColorScale(d.type)).attr("opacity", 0.7);
    // Update (and Enter transition)
    crimeEnter.merge(crimeCircles).transition().duration(500).attr("cx", (d)=>d.x).attr("cy", (d)=>d.y).attr("r", 3) // Small radius for scatter
    ...
    crimeEnter.on("mouseover", function(event, d) {
        ...
        tooltip.style("display", "block").html(`<strong>${d.type}</strong><br>Lat: ${d.lat.toFixed(3)}<br>Lon: ${d.lon.toFixed(3)}`).style("left", event.pageX + 10 + "px").style("top", event.pageY - 10 + "px");
    }).on("mouseout", function() {
        d3.select(this).attr("r", 3).attr("stroke", "none");
        tooltip.style("display", "none");
    });
    // update airbnb points as hexbins for better density visualization
    const hexbinGenerator = d3.hexbin().x((d)=>d.x).y((d)=>d.y).radius(15).extent([
        [
            0,
            0
        ],
    ...
    const hexagons = airbnbLayer.selectAll(".airbnb-hex").data(hexbins, (d)=>`${d.x}-${d.y}`);
    hexagons.exit().transition().duration(400).style("opacity", 0).remove();
    const hexagonsEnter = hexagons.enter().append("path").attr("class", "airbnb-hex").attr("d", hexbinGenerator.hexagon()).attr("stroke", "#2E7D32").attr("stroke-width", 0.5).style("opacity", 0).on("mouseover", function(event, d) {
        d3.select(this).transition().duration(150).style("stroke-width", 2).style("opacity", 0.9);
        ...
        d3.select(this).transition().duration(150).style("stroke-width", 0.5).style("opacity", 0.6);
        tooltip.style("display", "none");
    });
    hexagonsEnter.merge(hexagons).transition().duration(600).attr("transform", (d)=>`translate(${d.x},${d.y})`).attr("fill", (d)=>globalAirbnbScale(d.length)).style("opacity", 0.6);
}
// setup controls
function setupControls() {
    ...
    playBtn.on("click", function() {
        ...
        if (isPlaying) animationInterval = setInterval(()=>{
            currentPeriodIndex++;
            if (currentPeriodIndex >= data.periods.length) currentPeriodIndex = 0;
...
async function initialize() {
    try {
        ...
        globalAirbnbScale = d3.scaleSequential(d3.interpolateGreens).domain([
            0,
            maxAirbnbCluster
        ]);
        console.log(`Global scales - AirBnB cluster max: ${maxAirbnbCluster}`);
...
//# sourceMappingURL=chicago-map.ce46c198.js.map
